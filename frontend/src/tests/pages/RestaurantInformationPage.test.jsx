...
describe("RestaurantInformationPage", () => {
    const restaurantProfile = {
        id: 1, name: "La Marina Bistro", description: "Mediterranean restaurant near the seafront",
        phone: "+34 900 000 001", mondayWorkingHours: "09:00-22:00", tuesdayWorkingHours: "09:00-22:00",
        wednesdayWorkingHours: "09:00-22:00", thursdayWorkingHours: "09:00-22:00", fridayWorkingHours: "09:00-23:00",
        saturdayWorkingHours: "10:00-23:00", sundayWorkingHours: "10:00-22:00", cuisineCategoryNames: ["Italian"],
        localizations: [{ city: "Bilbao", province: "Bizkaia", country: "Spain", postalCode: "48001", number: "12", longitude: "2,5", latitude: "43,2" }],
    };

    const dashboard = {
        restaurantId: 1, totalOrders: 12, activeOrders: 3, cancelledOrders: 2,
        deliveredOrders: 7, totalRevenue: 123.45, totalRefunded: 9.99,
        recentOrders: [], 
    };
    ...
    test("renders profile data and normalizes coordinates with dot decimals", async () => {
        render(<RestaurantInformationPage />);

    
        expect(await screen.findByRole("heading", { name: /Restaurant Profile/i })).toBeInTheDocument();
        await waitFor(() => {
    ...
    test("pads working hours on blur and submits edited profile payload", async () => {
        ...
        await screen.findByRole("heading", { name: /Restaurant Profile/i });
        await waitFor(() => {
            expect(screen.queryByText(/Loading your restaurant data/i)).not.toBeInTheDocument();
        });
        ...
        expect(payload.localizations).toEqual([
            expect.objectContaining({
                city: "Bilbao", province: "Bizkaia", country: "Spain",
                postalCode: "48001", number: "12", longitude: 2.75, latitude: 43.11,
            }),
        ]);
